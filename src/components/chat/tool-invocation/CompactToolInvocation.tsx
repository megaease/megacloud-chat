...
export function CompactToolInvocation({
  ...
  const handleOpenArtifact = () => {
    ...
    // compact 模式下切换版本，正常模式下打开 artifact
    if (isCompact) {
      // 在 compact 模式下，切换到该版本
      const documentInfo = extractDocumentInfo(part);
      if (documentInfo?.version) {
        // 触发版本切换
        const versionSwitchEvent = new CustomEvent("artifact-version-switch", {
          detail: {
            documentId: args.documentId || documentInfo.documentId,
            version: documentInfo.version,
            title: args.title || documentInfo.title,
            kind: args.kind || documentInfo.kind,
          },
        });
        window.dispatchEvent(versionSwitchEvent);
      }
    } else {
      // 正常模式下打开 artifact
      const boundingBox = {
  ...
  if (
    ...
    // 检查是否已经有 documentId（对于 update-artifact）
    const hasDocumentId = !!args.documentId || !!resultTitle;
    ...
    return (
      <motion.div
        ...
        className={cn(
          ...
          // 只要有 documentId 或者标题就可以点击，不限制状态
          hasDocumentId &&
            !isDisabled &&
            !isStreaming &&
      ...
      >
        ...
        <div className="flex items-center gap-3 p-3 relative w-full">
          ...
          {/* 添加外部链接图标，表示可点击 */}
          {hasDocumentId && !isDisabled && !isStreaming && (
            <IconExternalLink
              size={14}
